...
APPS: tuple[AppLink, ...] = (
    AppLink("portfolio", "Portfolio Overview", "Multi-wallet balances across EVM chains, Lighter, Hyperliquid, LIT staking, and pool deposits.", _app_url("portfolio", 8790), "http://127.0.0.1:8790/api/summary", "Portfolio", "Wallet data stays in private SQLite storage on the VM."),
    AppLink("tracker", "Trade Tracker", "Live bot monitor for sources, positions, trade events, PnL cards, and system health.", _app_url("dashboard", 8080), "http://127.0.0.1:8080/healthz", "Trading", "Uses the deployed config and connected sources."),
    ...
    AppLink("hack-alert", "DeFi Hack Alert", "Protocol monitoring, anomaly detection, incident correlation, and provider health across major lending markets.", _app_url("hack", 8788), "http://127.0.0.1:8788/health/live", "Security", "Running in dry-run mode until separate Telegram credentials are configured."),
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Lighter Apps Hub</title>
<style>
...
</style></head><body><main><section class="top"><div><div class="eyebrow">Central control centre</div><h1>Lighter Apps Hub</h1><p class="intro">Open and monitor every deployed dashboard from one page.</p></div><button id="refresh" type="button">Refresh status</button></section>
...
<section class="grid">{_cards()}</section><p class="footer">Services run independently on the GCP VM; this hub checks their private health endpoints every 15 seconds.</p></main>
